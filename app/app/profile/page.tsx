...
export default async function ProfilePage() {
  ...
  return <div className="page"><div className="profile-hero"><span className="avatar profile-avatar">{session.initials}</span><div><div className="eyebrow light"><span/> beveiligd spelersaccount</div><h1>{session.displayName}</h1><p>{session.clubName} · @{session.username}</p></div></div><div className="profile-stats"><div className="card"><Trophy/><span><strong>{row.best === null ? "—" : row.best > 0 ? `+${row.best}` : row.best}</strong><small>Beste ronde</small></span></div><div className="card"><Award/><span><strong>{session.initials}</strong><small>Speler</small></span></div><div className="card"><Clock3/><span><strong>{row.rounds}</strong><small>Goedgekeurde rondes</small></span></div></div><ProfileSettings initialUsername={session.username || ""}/></div>;
}
