...
const createSchema = z.object({
  ...
  holeCount: z.union([z.literal(9), z.literal(18)]).optional(),
...
export async function POST(request: Request) {
  try {
    ...
    const roundId = await withTransaction(async (client) => {
      const course = await client.query<{ id: string; hole_count: 9 | 18; tee_id: string; pars: number[]; configured_holes: number }>(
        `SELECT c.id, c.hole_count, t.id AS tee_id, array_agg(h.par ORDER BY h.number) AS pars, count(h.id)::int AS configured_holes
      ...
      const roundHoleCount = body.holeCount ?? selected.hole_count;
      const totalPar = calculateRoundPar(selected.pars.map((par) => ({ par })), roundHoleCount);
      ...
      const inserted = await client.query<{ id: string }>(
        ...
        [body.clientId, session.clubId, selected.id, selected.tee_id, session.accountId, roundHoleCount, body.playerIds.length > 1 ? "inviting" : "active"],
      );
      ...
      for (const [index, playerId] of body.playerIds.entries()) {
        await client.query(
          ...
          [id, session.clubId, playerId, names.get(playerId), index + 1, totalPar,
      ...
      await client.query(
        `INSERT INTO audit_log(club_id,actor_role,actor_id,action,entity_type,entity_id,metadata)
         VALUES ($1,'player',$2,$5,'round',$3,jsonb_build_object('participants',$4::int,'holes',$6::int))`,
        [session.clubId, session.accountId, id, body.playerIds.length, body.playerIds.length > 1 ? "round.invitations_created" : "round.started", roundHoleCount],
      );
      return id;
    });
