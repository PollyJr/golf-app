...
export async function POST(request: Request) {
  try {
    ...
    const result = await query<{ id: string }>(
      `INSERT INTO players(club_id, display_name, initials, code, pin_hash) VALUES ($1, $2, $3, $4, $5) RETURNING id`,
      [session.clubId, body.displayName, initials, code, await hashSecret(pin)],
    );
    ...
    return NextResponse.json({ player: { id: result.rows[0].id, displayName: body.displayName, code, pin } }, { status: 201 });
  } catch (error) { return apiError(error); }
}
